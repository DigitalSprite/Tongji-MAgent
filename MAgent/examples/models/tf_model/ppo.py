...
class PPO(TFBaseModel):
    def __init__(self, env, handle, name, learning_rate=1e-3,
        ...
        custom_view_space: tuple
            customized feature space
        """
        TFBaseModel.__init__(self, env, handle, name, "tfppo")
        # ======================== set config  ========================
        self.env = env
        self.handle = handle
        self.name = name
    ...
    def _create_network(self, view_space, feature_space):
        ...
        old_policy = tf.layers.dense(dense, units=self.num_actions, activation=tf.nn.softmax)
        old_policy = tf.clip_by_value(policy, 1e-10, 1-1e-10)
        value = tf.layers.dense(dense, units=1)
        value = tf.reshape(value, (-1,))
        advantage = tf.stop_gradient(reward - value)
        ratio = policy/(old_policy + 1e-10)
        ...
        pg_loss = -tf.reduce_mean(tf.minimum(surr, tf.clip_by_value(ratio, 1 - LOSS_CLIPPING, 1 + LOSS_CLIPPING) * advantage))
        vf_loss = tf.reduce_mean(tf.square(reward - value))
    ...
    def get_info(self):
        ...
        info: string
        """
        return "ppo train_time: %d" % (self.train_ct)
