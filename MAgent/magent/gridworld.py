...
class GridWorld(Environment):
    ...
    def add_terrain(self, terrain_type, xs, ys):
        '''add terrain to environment

        Parameters:
        ----------
        terrain_type: str
            can be 'water', 'wall' or other terrain types
        xs: list(int)
            x-coordinates of terrain units
        ys: list(int)
            y-coordinates of terrain units
        '''
        terrain_tag = 0
        if terrain_type is 'water':
            xs, ys = np.array(xs), np.array(ys)
            _LIB.gridworld_add_terrain(self.game, ctypes.c_int32(terrain_tag), as_int32_c_array(xs), as_int32_c_array(ys))
        elif terrain_type is 'wall':
